...
class AddBevelGear(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        origZ = addMesh.GearFuncs.getOriginZ(self.module, self.nTeeth1, 'bevel', self.shiftX1,
                                             angcon1) + 1.25 * self.module * sin(angcon1) + (self.module / 2) * sin(
            angcon1) + bpy.context.scene.cursor_location[2]
...
class AddWorm(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        base2 = addMesh.GearFuncs.create_mesh_obj(context, mesh2)
        ob2 = bpy.context.active_object
        ob2.location.x = ob2.location.x - (addMesh.GearFuncs.getTipDiam(self.module, self.nTeeth) + self.dWorm) / 2
        ob2['nTeeth'] = self.nTeeth
        ...
        bpy.ops.object.add(type='EMPTY', location=(-(addMesh.GearFuncs.getTipDiam(self.module, self.nTeeth) + self.dWorm) / 2, 0, 0), rotation=(0.0, 0.0, 2*pi/self.nTeeth))
