...
def get_point_profile(m, nTeeth, pressureAngle, typeGear, shiftX, uMinEv, uMinFt, val, side):
    ...
    # bpy.ops.object.empty_add(type='PLAIN_AXES', radius=0.04, view_align=False, location=v3)
    return v3
...
def get_point_w_profile(m, nTeeth, dWorm, H, nTWorm, pressureAngle, shiftX, uMinEv, uMinFt, val, side, teeth):
    ...
    vertsEtalon2.clear()
    if side == 0:
        # bpy.ops.object.empty_add(type='PLAIN_AXES', radius=0.04, view_align=False, location=vt)
        return vt
    else:
        # bpy.ops.object.empty_add(type='PLAIN_AXES', radius=0.04, view_align=False, location=vt2)
        return vt2
